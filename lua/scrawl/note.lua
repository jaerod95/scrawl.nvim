...
function M.capture()
  ...
  if ctx.selection then
    ...
    vim.ui.input({ prompt = "Note: " }, function(input)
      ...
      send.text(string.format("/scrawl:note [%s:%s]\n%s```%s\n%s\n```", ctx.file, lines, note, lang, ctx.selection))
    end)
    return
  end
  ...
  vim.ui.input({ prompt = "Note: " }, function(input)
    if input and input ~= "" then
      send.text(string.format("/scrawl:note [%s:%d] %s", ctx.file, ctx.line, input))
    else
      send.text(string.format("/scrawl:note [%s:%d]", ctx.file, ctx.line))
    end
  end)
end
...
function M.decision()
  ...
  if ctx.selection then
    ...
    vim.ui.input({ prompt = "Decision: " }, function(input)
      ...
      send.text(string.format("/scrawl:decision [%s:%s] %s\n```%s\n%s\n```", ctx.file, lines, input, lang, ctx.selection))
    end)
    return
  end
  ...
  vim.ui.input({ prompt = "Decision: " }, function(input)
    if not input or input == "" then return end
    send.text(string.format("/scrawl:decision [%s:%d] %s", ctx.file, ctx.line, input))
  end)
end
